...
type Configuration struct {
	Group    `yaml:",inline"` // Group defines a single collection of org-mode documents.
	Subgroup map[string]Group `yaml:"subgroup"` // Subgroup are named collections of org-mode documents within the greater group.
	Tools    Tools            `yaml:"tools"`    // Tools configures the companion editor.
}

// Group defines a single group of org-mode documents and their configuration.
type Group struct {
